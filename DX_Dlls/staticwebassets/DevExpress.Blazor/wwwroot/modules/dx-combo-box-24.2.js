import { _ as __decorate } from './tslib.es6-24.2.js';
import adaptiveDropdownComponents from './adaptivedropdowncomponents-24.2.js';
import { j as comboBox2TagName } from './constants-24.23.js';
import { D as DxDropDownListBox } from './dx-dropdown-list-box-24.2.js';
import { E as EventHelper } from './eventhelper-24.2.js';
import { n } from './property-24.2.js';
import './dropdowncomponents-24.2.js';
import './dropdown-24.2.js';
import './popup-24.2.js';
import './layouthelper-24.2.js';
import './point-24.2.js';
import './constants-24.2.js';
import './rafaction-24.2.js';
import './screenhelper-24.2.js';
import './transformhelper-24.2.js';
import './positiontracker-24.2.js';
import './branch-24.2.js';
import './custom-element-24.2.js';
import './lit-element-24.2.js';
import './logicaltreehelper-24.2.js';
import './portal-24.2.js';
import './data-qa-utils-24.2.js';
import './constants-24.22.js';
import './const-24.2.js';
import './dx-html-element-pointer-events-helper-24.2.js';
import './dom-24.2.js';
import './_commonjsHelpers-24.2.js';
import './string-24.2.js';
import './browser-24.2.js';
import './devices-24.2.js';
import './dx-ui-element-24.2.js';
import './lit-element-base-24.2.js';
import './dx-license-24.2.js';
import './capture-manager-24.2.js';
import './focushelper-24.2.js';
import './nameof-factory-24.2.js';
import './custom-events-helper-24.2.js';
import './focustrap-24.2.js';
import './tabbable-24.2.js';
import './key-24.2.js';
import './keyboard-navigation-strategy-24.2.js';
import './dom-utils-24.2.js';
import './css-classes-24.2.js';
import './focus-utils-24.2.js';
import './disposable-24.2.js';
import './thumb-24.2.js';
import './query-24.2.js';
import './popupportal-24.2.js';
import './events-interseptor-24.2.js';
import './modalcomponents-24.2.js';
import './dx-dropdown-base3-24.2.js';
import './masked-input-24.2.js';
import './text-editor-24.2.js';
import './single-slot-element-base-24.2.js';
import './input-24.2.js';
import './constants-24.25.js';
import './dx-scroll-viewer-24.2.js';
import './dx-html-element-base-24.2.js';
import './scroll-viewer-css-classes-24.2.js';
import './dx-list-box-events-24.2.js';
import './dx-virtual-scroll-viewer-24.2.js';
import './grid-scroll-utils-24.2.js';

var DropDownTriggerMode;
(function (DropDownTriggerMode) {
    DropDownTriggerMode["Input"] = "Input";
    DropDownTriggerMode["Click"] = "Click";
})(DropDownTriggerMode || (DropDownTriggerMode = {}));
class DxComboBox2 extends DxDropDownListBox {
    constructor() {
        super();
        this.dropDownTriggerMode = DropDownTriggerMode.Input;
    }
    connectedCallback() {
        super.connectedCallback();
    }
    disconnectedCallback() {
        super.disconnectedCallback();
    }
    applyTextPropertyCore() {
        super.applyTextPropertyCore();
        if (this.allowInput)
            return;
        if (this.fieldElement && this.fieldText && this.fieldText !== "")
            this.fieldElement.setSelectionRange(0, 0);
    }
    ensureDropDownOpened(evt) {
        if (!this.isDropDownOpen) {
            const containsData = evt.data && evt.data.length > 0;
            const containsValue = !this.fieldElement ? this.fieldText : this.fieldElementValue;
            if (containsData || containsValue)
                super.tryOpenDropDown();
        }
    }
    processPointerDown(e) {
        const isFieldElement = EventHelper.containsInComposedPath(e, this.isFieldElement.bind(this));
        const isEditBoxTemplateElement = EventHelper.containsInComposedPath(e, this.isEditBoxTemplateElement.bind(this));
        let shouldToggleDropDownVisibility = false;
        if (isEditBoxTemplateElement)
            shouldToggleDropDownVisibility = true;
        else if (isFieldElement)
            shouldToggleDropDownVisibility = this.dropDownTriggerMode === DropDownTriggerMode.Click || this.isFieldElementReadonly;
        if (shouldToggleDropDownVisibility)
            this.toggleDropDownVisibility();
        return super.processPointerDown(e);
    }
}
__decorate([
    n({ type: DropDownTriggerMode, attribute: "drop-down-trigger-mode" })
], DxComboBox2.prototype, "dropDownTriggerMode", void 0);
customElements.define(comboBox2TagName, DxComboBox2);
function loadModule() { }
const dxComboBox = { loadModule, adaptiveDropdownComponents };

export { DropDownTriggerMode, DxComboBox2, dxComboBox as default };
//# sourceMappingURL=dx-combo-box-24.2.js.map
